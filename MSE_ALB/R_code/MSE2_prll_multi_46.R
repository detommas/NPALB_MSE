...
for (h in 1:length(hn)){
  ...
  foreach(itr = 1:1, .packages = c('r4ss','Rcpp','dplyr','reshape2'),.noexport = "ssf2_V2.cpp") %dopar% { NPALB_MSE2_hs3_tvry(hsnum,hcrnum,scnnum,itr, rdev1 = -4.35459e-06, Bthr = Bthr, Blim = Blim,Tmin=Tmin, pl = pl) }
