...
sdir = "C:/Users/FRDScientist/Documents/Desiree/MSE_2/MSE_ALB/Condition/"
...
#below we specify the year after those haapen in the 30 years time series, i.e. when a new TAC is applied
asmt_t = seq(1, 30, by=3)

#specify the frequency of assessements- really it is how often the data needs to be generated, yearly for tae control as the catches are dependent on the biomass
tasmt = 3
...
pr = c(0.8,rep(0.9,7),0.8,rep(0.9,7))
  ...
  #Run the MSE code for 70 iterations
  ...
  foreach(itr = 1:70, .packages = c('r4ss','Rcpp','dplyr','reshape2'),.noexport = "ssf2_V2.cpp") %dopar% { NPALB_MSE2_hs32_tvry(hsnum,hcrnum,scnnum,itr, rdev1 = -4.35459e-06, Bthr = Bthr, Blim = Blim,Tmin=Tmin, pl = pl)}
