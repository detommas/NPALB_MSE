...
NPALB_MSE2_hs3_tvry_tae_rhist = function(hsnum,hcrnum,scnnum,itr, rdev1, Bthr, Blim, Tmin, pl) { 
...
TAEi = 1:30 # current total TAE with implementation error
...
for (tstep in 1:30){
  ...
  if (tstep == 1) {
     ...
     OMBoot_fun_tvry_tactae(pdir, sdir, hs, hcr, scn, hsw, hcrw, scnw, pwin, itr, tstep, tasmt, new_cdat, rec_devs, sel_devs)
     ...
     TAC_mat = HCR3_TAC2_interim_tae(dat=SPRmatb, yr=yr_endb, tae=tae1,tac=tac1, taci=tac1, cr=cr_hcr)
    ...
    if (tstep %in% seq(3,30,3)){
      ...
      EM_fun_nohess_tactae(pdir, sdir, hs, hcr, scn, hsw, hcrw, scnw, pwin, itr, tstep)
      ...
      TAC_mat = HCR3_TAC2_tae_rhist(myreplist = em_out, dat = SPRmat, datom = SPRmatbom, yr=yr_end, yrom =yr_endbom,SSBtrs=ssb_thr, SSBlim=ssb_lim, Ftgt=em_ftgt, TACmin=Tmin, TACmax=120000, plim = pl, err = imp_e[tstep+1],cr=cr_hcr,pdir, sdir, hs, hcr, scn, hsw, hcrw, scnw, pwin, itr, tstep,hcrnum)
      file_tac = paste(pdir, hs, hcr, scn, itr,"/TAC",(tstep+1),".txt", sep = "")
      write.table(TAC_mat, file_tac)
      ...
      #function to calculate the catch and tae by fleet. No implementation error for this first step
      TAC_mat = HCR3_TAC2_interim_tae(dat=SPRmatb, yr=yr_endb, tae=TAEi[tstep],tac=TACdt[tstep], taci = TACi[tstep],cr=cr_hcr)
