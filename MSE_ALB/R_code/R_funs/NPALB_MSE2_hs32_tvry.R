...
NPALB_MSE2_hs32_tvry = function(hsnum,hcrnum,scnnum,itr, rdev1, Bthr, Blim, Tmin, pl) { 
...
B0t = 1:30 # current dynamic SSB0 from em
B0t_om = 1:30 # current dynamic SSB0 from om
...
for (tstep in 1:10){
  ...
  #***************************************************************************
  #Step 2: Run OM and estimate data with error and no error as input to EM and OM via SS Bootstrap routine
  OMBoot_fun_tvry(pdir, sdir, hs, hcr, scn, hsw, hcrw, scnw, pwin, itr, tstep, tasmt, new_cdat, rec_devs, sel_devs)
  
  #****************************************************************************
  #Step 3: Save all the OM files in a separate folder
  ...
  TAC_mat = HCR3_TAC2_rhist(myreplist = em_out, dat = SPRmat, yr=yr_end, SSBtrs=ssb_thr, SSBlim=ssb_lim, Ftgt=em_ftgt, TACmin=Tmin, TACmax=120000, plim = pl,pdir, sdir, hs, hcr, scn, hsw, hcrw, scnw, pwin, itr, tstep,hcrnum)
  ...
  B0dat[asmt_t[tstep]:(asmt_t[tstep]+2)] = DBzero_lat_em$SSB_nofishing
  ...
  SPB_em[asmt_t[tstep]:(asmt_t[tstep]+2)] = SPRmat$SSB[(dim(SPRmat)[1]-2):dim(SPRmat)[1]]
  ...
  B0t[asmt_t[tstep]:(asmt_t[tstep]+2)] = (em_out$derived_quants %>% filter(Label == "SmryBio_Unfished"))$Value
...
SPBdat[1:(tasmt*tstep)] = derquant$SSB[st:length(derquant$SSB)]
