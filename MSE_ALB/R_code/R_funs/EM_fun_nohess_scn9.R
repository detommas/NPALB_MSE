...
EM_fun_nohess_scn9 <- function(pdir, sdir, hs, hcr, scn, hsw, hcrw, scnw, pwin, itr, tstep){
...
#*****************************CHANGE DAT FILE*******************************************   
  ...
  if (tstep ==1){
    ...
    boot_old$catch$F25[92:95]=boot_old$catch$F25[92:95]-600
    ...
    if (tstep>1&tstep<4){
      f25c = 14400*(tstep-1)
      boot_dat$catch$F25[indi:(indi+4)]=boot_dat$catch$F25[indi:(indi+4)]-(f25c/4)-600
      boot_dat$catch$F25[(indi+5):(indi+8)]= boot_dat$catch$F25[(indi+5):(indi+8)]-(f25c/4)-(600*2) 
      boot_dat$catch$F25[(indi+9):indf]=boot_dat$catch$F25[(indi+9):indf]-(f25c/4)-(600*3)
    } else {
      f25c = 50000
