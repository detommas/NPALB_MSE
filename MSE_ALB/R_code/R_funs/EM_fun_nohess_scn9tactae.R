...
EM_fun_nohess_scn9tactae <- function(pdir, sdir, hs, hcr, scn, hsw, hcrw, scnw, pwin, itr, tstep){
...
#*****************************CHANGE DAT FILE*******************************************   
  ...
  boot_dat=SS_readdat(file = "data.ss_new", section = 3, version = "3.24")
  
  #read the old bootstrap data file (i.e. from previous time step). In the case of the first time assessment, happening at tstep=3, this is the original .dat file
  if (tstep ==3){
    boot_old = boot_dat
    boot_new = boot_old
    #remove the ghost fleet catches from the last three years, which are not seen by the assessment
    boot_old$catch$F25[92:95]=boot_old$catch$F25[92:95]-(2400/4)
    boot_old$catch$F25[96:99]=boot_old$catch$F25[96:99]-(600*2)
    boot_old$catch$F25[100:104]=boot_old$catch$F25[100:104]-(600*3)
    boot_new = boot_old
    
  } else {
    boot_file = paste(pdir, hs, hcr, scn, itr,"/",(tstep-3),"/EM/EMdat.ss", sep="")
    ...
    #need to remove the ghost fleet catches that were part of the dataset
    if (tstep<21){
      f25c = 2400*(tstep)
      boot_dat$catch$F25[indi:(indi+4)]=boot_dat$catch$F25[indi:(indi+4)]-(f25c/4)
      boot_dat$catch$F25[(indi+5):(indi+8)]= boot_dat$catch$F25[(indi+5):(indi+8)]-(f25c/4)-600 
      boot_dat$catch$F25[(indi+9):indf]=boot_dat$catch$F25[(indi+9):indf]-(f25c/4)-(600*2)
    } else {
      f25c = 50000
      boot_dat$catch$F25[indi:(indi+4)]=boot_dat$catch$F25[indi:(indi+4)]-(f25c/4)
      boot_dat$catch$F25[(indi+5):(indi+8)]= boot_dat$catch$F25[(indi+5):(indi+8)]-(f25c/4)
      boot_dat$catch$F25[(indi+9):indf]=boot_dat$catch$F25[(indi+9):indf]-(f25c/4)
    }
  ...
  command_mv = paste("cp forecast.ss ", pdir, hs, hcr, scn, itr, "/",tstep,"/EM/", sep ="")
