...
NPALB_MSE2_hs3_tvry_tactae = function(hsnum,hcrnum,scnnum,itr, rdev1, Bthr, Blim, Tmin, pl) { 
...
for (tstep in 1:30){
  ...
  if (tstep == 1) {
    ...
    if (tstep %in% seq(3,30,3)){
      ...
      TAC_mat = HCR3_TAC2_tactae(myreplist = em_out, dat = SPRmat, datom = SPRmatbom, yr=yr_end, yrom=yr_endbom,SSBtrs=ssb_thr, SSBlim=ssb_lim, Ftgt=em_ftgt, TACmin=Tmin, TACmax=120000, plim = pl, err = imp_e[tstep+1],cr=cr_hcr)
