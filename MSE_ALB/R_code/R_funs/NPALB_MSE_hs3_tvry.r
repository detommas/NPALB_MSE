...
NPALB_MSE_hs3_tvry = function(hsnum,hcrnum,scnnum,itr, rdev1, Bthr, FBthr, Blim) { 
...
pdir = "C:/Users/FRDScientist/Documents/Desiree/MSE_ALB/"
pwin = "C:\\Users\\FRDScientist\\Documents\\Desiree\\MSE_ALB\\"

#Specify the path of conditioned initial OM
sdir = "C:/Users/FRDScientist/Documents/Desiree/MSE_ALB/Condition/"
...
TACflt = matrix(data = NA, nrow = 10, ncol = 30) #TAC by fishery
...
imp_e = implem_err(itr, n=30, nf=10, sigma=0.05)
...
for (tstep in 1:10){
  ...
  #Rdat[asmt_t[tstep]:(asmt_t[tstep]+2)] = derquant$Recruits[(length(derquant$Recruits)-2):length(derquant$Recruits)]
  #SPBdat[asmt_t[tstep]:(asmt_t[tstep]+2)] = derquant$SSB[(length(derquant$SSB)-2):length(derquant$SSB)]
  #Ddat[asmt_t[tstep]:(asmt_t[tstep]+2)] = derquant$Deplete[(length(derquant$Deplete)-2):length(derquant$Deplete)]
  #SPRdat[asmt_t[tstep]:(asmt_t[tstep]+2)] = derquant$SPR[(length(derquant$SPR)-2):length(derquant$SPR)]
  #Btot[asmt_t[tstep]:(asmt_t[tstep]+2)] = derquant[(length(derquant$SPR)-2):length(derquant$SPR),15] #Bio_Smry
  #Tdat[asmt_t[tstep]:(asmt_t[tstep]+2)] = derquant$Retain_Catch[(length(derquant$SPR)-2):length(derquant$SPR)]
  TACdt[asmt_t[tstep]:(asmt_t[tstep]+2)] = rep(TACdat,3)
  #Ftgt_om [asmt_t[tstep]:(asmt_t[tstep]+2)] = rep(om_out$derived_quants$Value[which(om_out$derived_quants$Label == "Fstd_SPRtgt")],3)
  ...
  #select the catch for the latest 3 years
  #catch_lat = catch %>% filter(Yr== (yr_end-2) |Yr== (yr_end-1) |Yr== yr_end)
  
  #sum catch across seasons
  #catch_sum = as.data.frame(catch_lat %>% group_by(Yr,Name) %>% summarise(cat=sum(Exp)))
  
  #set to wide format to match other data, so that year is in rows anch column is the catch by fishery
  #catch_wide <- dcast(catch_sum, Yr ~ Name, value.var="cat")
  
  #Cdat[asmt_t[tstep]:(asmt_t[tstep]+2),] = as.matrix(catch_wide)
  
  #extract dynamic bzero data
  #DBzero = om_out$Dynamic_Bzero
  #DBzero_lat = DBzero %>% filter(Yr== (yr_end-2) |Yr== (yr_end-1) |Yr== yr_end)
  #B0dat_om[asmt_t[tstep]:(asmt_t[tstep]+2)] = DBzero_lat$SPB_nofishing
  
  #****************************************************************************
  #Step 5: Run the estimation model
  EM_fun_nohess(pdir, sdir, hs, hcr, scn, hsw, hcrw, scnw, pwin, itr, tstep)
  
  #****************************************************************************
  #Step 6: Compute TAC using EM model output
  ...
  ssb_thr = brp_fun(ssoutput=em_out, iyear=(2012+3*tstep), fyear=(2014+3*tstep), fraction=(FBthr*Bthr))
  ...
  TAC_mat = HCR3_TAC_byfleet(dat = SPRmat, yr=yr_end, SSBtrs=ssb_thr, SSBlim=ssb_lim, Ftgt=em_ftgt, TACmin=0,cr=cr_c, err=imp_e[tstep,])

  #Save the TAC
  file_tac = paste(pdir, hs, hcr, scn, itr,"/TAC",(tstep+1),".txt", sep = "")
  write.table(TAC_mat$TAC, file_tac)
  ...
  TACflt[,c(asmt_t[tstep]:(asmt_t[tstep]+2))] = as.matrix(cbind(TAC_mat$TAC_flt,TAC_mat$TAC_flt,TAC_mat$TAC_flt))
  
}

#Run the OM at the last time step
