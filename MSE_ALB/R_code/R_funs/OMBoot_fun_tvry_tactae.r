...
OMBoot_fun_tvry_tactae <- function(pdir, sdir, hs, hcr, scn, hsw, hcrw, scnw, pwin, itr, tstep, tasmt, new_cdat, rec_devs, sel_devs){
...
#*****************************CHANGE DAT FILE*******************************************   
  ...
  if (sum(new_cdat[,2:30])==0){
    ...
    change_dat_cpue_boot_tactae(ss_file_in = file_in, ss_file_out = file_out, t_asmt = tasmt, nrep = tasmt, cdat_new = new_cdat)
  }
