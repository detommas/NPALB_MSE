...
NPALB_MSE2_hs32_tvry_tactae = function(hsnum,hcrnum,scnnum,itr, rdev1, Bthr, Blim, Tmin, pl) { 
...
for (tstep in 1:30){
  ...
  if (tstep %in% seq(3,30,3)){
    ...
    TAC_mat = HCR3_TAC2_tactae_rhist(myreplist = em_out, dat = SPRmat, yr=yr_end, SSBtrs=ssb_thr, SSBlim=ssb_lim, Ftgt=em_ftgt, TACmin=Tmin, TACmax=120000, plim = pl, err = imp_e[tstep+1])
