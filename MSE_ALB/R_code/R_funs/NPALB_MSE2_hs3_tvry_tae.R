...
NPALB_MSE2_hs3_tvry_tae= function(hsnum,hcrnum,scnnum,itr, rdev1, Bthr, Blim, Tmin, pl) { 
...
for (tstep in 1:30){
  ...
  if (tstep == 1) {
    ...
    if (tstep %in% seq(3,30,3)){
      ...
      TAC_mat = HCR3_TAC2_tae_norhist(myreplist = em_out, dat = SPRmat, datom = SPRmatbom, yr=yr_end, yrom =yr_endbom,SSBtrs=ssb_thr, SSBlim=ssb_lim, Ftgt=em_ftgt, TACmin=Tmin, TACmax=120000, plim = pl, err = imp_e[tstep+1],cr=cr_hcr,pdir, sdir, hs, hcr, scn, hsw, hcrw, scnw, pwin, itr, tstep,hcrnum)
      file_tac = paste(pdir, hs, hcr, scn, itr,"/TAC",(tstep+1),".txt", sep = "")
      write.table(TAC_mat, file_tac)
